    ...
    <header className={`bg-[#f3f4f6] text-black py-4 pb-2 fixed top-0 left-0 right-0 z-10 transition-all ease-in-out duration-300 ${!headerVisible ? 'transform -translate-y-full' : ''}`}>
      <div className="container mx-auto flex justify-between items-center px-6 md:px-12 lg:px-20">
        ...
        <nav className="hidden md:flex space-x-4 items-center">
          ...
          <Link href="/about" className="hover:text-[#0085FF]">About</Link>
          <Link href="/products" className="bg-[#0085FF] text-white font-semibold py-2 px-5 rounded-md shadow-md hover:bg-[#005FCC] transition ease-in-out duration-300">
            Explore Now
          </Link>
        </nav>
      ...
      <div className={`md:hidden ${isMenuOpen ? 'block' : 'hidden'}`}>
        <nav className="space-y-4 px-6 py-4">
          ...
          <Link href="/about" className="block hover:text-[#0085FF]">About</Link>
          <Link href="/products" className="block bg-[#0085FF] text-white font-semibold py-2 px-5 rounded-md shadow-md hover:bg-[#005FCC] transition ease-in-out duration-300">
            Explore Now
          </Link>
        </nav>
      </div>
